 ...
 * <dt><code>m|? to n|? multiples|powers|any</code></dt>
 ...
 * <dt><code>multiples|powers of p</code></dt>
 ...
 * specified.</dd>
 * <dt><code>multiples|powers|distinct|any</code></dt>
 ...
 * </dl>
 */
final class _PropertyValueGrouperParser
    extends Parser<PropertyValueGrouper> {
  ...
  public final PropertyValueGrouper parseString(final String string) {
    ...
    try {
      ...
      define: {
        ...
        if (minMaxMode) {
          current = iterator.next();
          if (!(_PropertyValueGrouperParser.TO
            ...
            switch (TextUtils.toLowerCase(current)) {
              ...
              default: {
                ...
                "Found '" + current) + //$NON-NLS-1$
                    ...
                    + _PropertyValueGrouperParser.MULTIPLES) + ',') + ' ')
                    + _PropertyValueGrouperParser.POWERS)//
                    + ", or ") + //$NON-NLS-1$
          ...
          if (iterator.hasNext()) {
            ...
            current = iterator.next();
            if (!(_PropertyValueGrouperParser.OF
                .equalsIgnoreCase(current))) {
              throw new IllegalArgumentException((((//
      ...
      if (Compare.equals(mode, defMode) && //
          (minGroups == defMinGroups) && //
          (maxGroups == defMaxGroups) && //
