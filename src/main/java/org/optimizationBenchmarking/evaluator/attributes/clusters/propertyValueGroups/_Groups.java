...
final class _Groups implements Comparable<_Groups> {
  ...
  _Groups(final _Group[] groups, final int minGroups, final int maxGroups,
    ...
    this.m_balanceScore = Math.sqrt(
        Div.INSTANCE.computeAsDouble(sumSqr, count) - (balance * balance));

    if ((mode != EGroupingMode.DISTINCT) && (discontinuous > 0)) {
      this.m_discontinuousScore = Div.INSTANCE
          .computeAsDouble(discontinuous, count);
    } else {
  ...
  public final int compareTo(final _Groups o) {
    ...
    if (((resCount == 0) || //
      ...
      if ((resDisc != 0) && //
          ((Math.max(this.m_discontinuousScore,
