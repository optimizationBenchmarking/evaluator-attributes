...
final class _BoundConstant extends _DataBasedConstant {
  ...
  private final Number __getDefaultBound() {
    ...
    if (this.m_defaultBound == null) {
      ...
      if (parser.areBoundsInteger()) {
        if (this.m_upper) {
          this.m_defaultBound = NumericalTypes
              .valueOf(parser.getUpperBoundLong());
        } else {
          this.m_defaultBound = NumericalTypes
              .valueOf(parser.getLowerBoundLong());
        }
      } else {
        if (this.m_upper) {
          this.m_defaultBound = NumericalTypes
              .valueOf(parser.getUpperBoundDouble());
        } else {
          this.m_defaultBound = NumericalTypes
              .valueOf(parser.getLowerBoundDouble());
        }
      }
    }
  ...
  public final String toString() {
    return ((this.m_upper ? //
        ...
        : "lower bound of dimension ") + this.m_dimension.getName()); //$NON-NLS-1$
  }
